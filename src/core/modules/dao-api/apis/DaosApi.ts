...
export class DaosApiRequestFactory extends BaseAPIRequestFactory {
     ...
     */
    public async toggleDAOFeatured(daoId: string, featuredToggle: FeaturedToggle, _options?: Configuration): Promise<RequestContext> {
        let _config = _options || this.configuration;

        // verify required parameter 'daoId' is not null or undefined
        if (daoId === null || daoId === undefined) {
            throw new RequiredError("DaosApi", "toggleDAOFeatured", "daoId");
        }


        // verify required parameter 'featuredToggle' is not null or undefined
        if (featuredToggle === null || featuredToggle === undefined) {
            throw new RequiredError("DaosApi", "toggleDAOFeatured", "featuredToggle");
        }
...
export class DaosApiResponseProcessor {
     ...
     * to the expected objects
     *
     * @params response Response returned by the server for a request to toggleDAOFeatured
     * @throws ApiException if the response code was not in [200, 299]
     */
     public async toggleDAOFeaturedWithHttpInfo(response: ResponseContext): Promise<HttpInfo<FeaturedResponse >> {
        const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
        if (isCodeInRange("422", response.httpStatusCode)) {
            const body: Error = ObjectSerializer.deserialize(
                ObjectSerializer.parse(await response.body.text(), contentType),
